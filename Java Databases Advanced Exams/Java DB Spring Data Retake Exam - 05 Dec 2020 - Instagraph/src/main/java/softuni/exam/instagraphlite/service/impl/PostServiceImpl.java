...
import softuni.exam.instagraphlite.models.dto.PictureSeedPathDTO;
import softuni.exam.instagraphlite.models.dto.PostSeedRootDTO;
import softuni.exam.instagraphlite.models.dto.UserSeedUsernameDTO;
        ...
        xmlParser.fromFile(POSTS_FILE_PATH, PostSeedRootDTO.class)
                ...
                .filter(postSeedDTO -> {
                    boolean isValid = validationUtil.isValid(postSeedDTO)
                            && userService.isEntityExist(postSeedDTO.getUsername().getUsername())
                            && pictureService.isEntityExist(postSeedDTO.getPictures().getPath());

                    builder.append(isValid ? String.format("Successfully imported Post, made by %s",
                            postSeedDTO.getUsername().getUsername())
                            : "Invalid Post")
                ...
                .map(postSeedDTO -> {
                    ...
                    post.setUser(userService.findByUsername(postSeedDTO.getUsername().getUsername()));
                    post.setPicture(pictureService.findByPath(postSeedDTO.getPictures().getPath()));
                    return post;
                })
